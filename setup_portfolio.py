...
# --- 2. Define the Humanized, Humble README Content ---
...
## Project Overview
This project bridges Clinical Psychology and AI Safety. It serves as a **preliminary empirical validation** of whether Large Language Models (LLMs) can maintain consistent psychological value profiles ("personas") when faced with moral dilemmas, without being explicitly defined.
...
This chart visualizes the model's personality profile. A balanced model would ideally exhibit a wide, even circle.
![Radar Chart](value_profile_radar_chart.png)

**Interpretation:**
- **Bimodal "Flip-Flop" Profile:** The radar chart suggests the model lacks a coherent circular profile. Instead, it appears to flip-flop between two extremes: **Tradition (0.38)** and **Hedonism (0.38)**.
- **Universalism Signal:** Note the weaker signal for Universalism compared to Stimulation, which may indicate semantic ambiguity in the model's output for this value.

### 2. Structural Consistency (Success vs. Failure)
This chart measures if the model successfully distinguished itself from its psychological opposite (e.g., Did 'Stimulation' sound distinct from 'Conservation'?).
...
- **Red Bars:** Failure (Score < 0). The model's response aligned more closely with the opposite value.

### 3. The "Power Bias" (Confusion Heatmap)
This heatmap visualizes potential semantic confusion in the embedding space.
![Confusion Heatmap](alignment_confusion_heatmap.png)

**Interpretation:**
- **The Anomaly:** Observe the vertical column for **Power**. It is consistently highlighted for multiple rows (Universalism, Benevolence, Self-Direction).
- **Conclusion:** This suggests the embedding model may conflate "Moral Authority" (enforcing rules) with "Dominance" (Power), creating a false positive for Power across ethical prompts.
